...
if systemctl is-active --quiet "${SERVICE_NAME}"; then
  ...
  echo ""
  echo -e "  📋 常用命令（随时随地可用）："
  echo -e "     查看状态: ${YELLOW}meshlink stats${NC}"
  echo -e "     寻址测试: ${YELLOW}meshlink test <虚拟IP>${NC}"
  echo -e "     查看日志: ${YELLOW}meshlink logs${NC}"
  echo -e "     重启服务: ${YELLOW}meshlink restart${NC}"
  echo -e "     彻底卸载: ${YELLOW}sudo bash uninstall.sh${NC}"
  echo ""
  echo -e "  📢 帮助手册: ${YELLOW}meshlink --help${NC}"
  echo ""
else
  warn "服务启动可能失败，请检查日志："
